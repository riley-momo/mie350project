...
//			stmt.execute(query);
	...
	public void changeRating(StudentUser student, Restaurant res, int rating){
		/**
		 * This method changes an existing rating in the database
		 */
		try {
			int studentID = student.getUserID();
			int resID = res.getResID();
	
			String query = "UPDATE Rating SET Rating = " + rating +" WHERE UserID = " + studentID + " AND RestaurantID = " + resID + ";";
			Statement stmt = connection.createStatement();
			stmt.execute(query);
